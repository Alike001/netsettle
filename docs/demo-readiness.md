...
- [x] Non-participant isolation: a fourth Ethereum Sepolia wallet inspected Round #1 without being able to fund, submit, validate, finalize, withdraw, or refund that round. The current UI lets such a wallet start a separate independent round instead of modifying the original group.
...
- [x] New-round entry: a connected observer opened refunded Round #2, selected “Start separate round,” and reached the real onchain group-creation form without console errors
- [ ] Four-minute-or-shorter recorded demo
- [ ] Local copy of the video and screenshots for offline presentation
- [x] Public repository clone/install check: `npm ci`, deterministic/frontend tests, Solidity compilation, and Vite production build completed with no project `.env`
