...
- Round #1 has now advanced to Collecting obligations with all three deposits confirmed. The first Account C encrypted submission surfaced `Unsupported client. Expected a viem WalletClient instance connected to an account.` The browser adapter now creates the documented Viem wallet client directly from MetaMask's injected provider and pins it to Wagmi's connected address. Its first Vercel build failed because a clean TypeScript build lacked an explicit `Window.ethereum` declaration; that declaration is now added. Clean build, lint, and ten app tests pass locally; deploy and real MetaMask/Nox retest are still required.
...
1. Deploy the browser-wallet adapter fix, then retry Account C's encrypted submission (4 to A, 1 to B) in Round #1.
2. Continue Round #1: submit B and A, reload/retry during Nox computation, validate, finalize, and withdraw with transaction evidence.
