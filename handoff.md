...
- Round #1 has now advanced to Collecting obligations with all three deposits confirmed. The first Account C encrypted submission surfaced `Unsupported client. Expected a viem WalletClient instance connected to an account.` The browser adapter fix is ready locally: it now creates the documented Viem wallet client directly from MetaMask's injected provider and pins it to Wagmi's connected address. App type-check, lint, ten unit tests, and production build pass; deploy and real MetaMask/Nox retest are still required.
